                  ...
                  <>
                    <div className="grid grid-cols-2 md:gap-4 justify-between col-span-2 w-full gap-2">
                      ...
                      {activities.map((activity) => (
                        ...
                        >
                          <article className="rounded-lg overflow-clip">
                            <Link href={`/p/activities/${activity.documentId}`}>
                              <div className="md:w-full md:max-w-full max-w-[196px] hover:shadow-md duration-200 min-w-[170px] w-full min-h-[250px] h-full bg-white items-start justify-start border rounded-3xl flex flex-col md:flex-row gap-4">
                                <div className="claracontainer w-full flex-col justify-start items-center gap-7 inline-flex">
                                  <div className="w-full max-w-full md:min-w-full lg:max-w-full h-auto">
                                    <div className="flex max-h-[180px] min-h-[150px] h-[150px] md:min-h-[200px] md:h-full lg:min-h-[276px] lg:h-full lg:max-h-[276px] md:max-h-[300px] overflow-clip rounded-t-3xl">
...
export async function AllActivitiesPage() {
  const activities = await fetchAllActivities();

  if (!activities) {
    notFound(); // If no activities found, show 404 page
  }
  ...
  return (
    <div className="activities-page">
      ...
      <div className="activity-cards-container">
        {activities.map((activity) => (
          <div key={activity.documentId} className="activity-card">
            ...
            <div className="activity-card-content">
              ...
              {/* Display Learning Area */}
              <p className="learning-area">
              ...
              <p
                className="skills"
                dangerouslySetInnerHTML={{ __html: activity.Skills }}
              />

              {/* Display Theme */}
              <p className="theme">Theme: {activity.Theme}</p>

              {/* Show Gallery Image (first image) */}
              {activity.Gallery?.data && activity.Gallery.data.length > 0 && (
                <Image
                  src={`http://localhost:1337${activity.Gallery.data[0].attributes.url}`} // Use 'attributes.url' for Strapi media
                  alt={activity.Title}
                  width={500}
                  height={300}
                  layout="intrinsic"
              ...
              {/* Optionally show the accordions or other repeatable components */}
              {activity.Accordions?.map((accordion, index) => (
