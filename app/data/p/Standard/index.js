...
export async function fetchTnc() {
  try {
    const response = await fetch("https://upbeat-life-04fe8098b1.strapiapp.com/api/tnc?populate=*");
    if (!response.ok) {
      throw new Error(`Error fetching data: ${response.statusText}`);
    }
...
export async function fetchFaq() {
  try {
    const response = await fetch("https://upbeat-life-04fe8098b1.strapiapp.com/api/faqs?populate=*");
    if (!response.ok) {
      throw new Error(`Error fetching data: ${response.statusText}`);
    }
